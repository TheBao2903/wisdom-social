...
export function useChatAI({ conversationId }: UseChatAIOptions) {
    ...
    const fetchConsentStatus = useCallback(async (): Promise<boolean> => {
        ...
        try {
            ...
            return response.confirmUseAI;
        } catch (error) {
            setAiError(parseAIError(error).message);
    ...
    const ensureCanUseAI = useCallback(async (): Promise<boolean> => {
        if (consentStatus === true) return true;
    ...
    const summarizeConversation = useCallback(
        async (currentMessages?: MessagePreviewDTO[]) => {
            setAiError(null);
            ...
            try {
                ...
                const payload = sanitizedMessages.length
                    ? { conversationId, messages: sanitizedMessages }
                    : { conversationId, limit: 30 };
                const response = await chatAIService.summarizeConversation(payload);
