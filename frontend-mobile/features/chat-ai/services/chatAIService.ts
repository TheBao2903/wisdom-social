...
const chatAIService = {
    async getConsentStatus(): Promise<ConfirmAIResponse> {
        const response = await apiClient.get<ApiResponse<ConfirmAIResponse>>(
            "/users/me/confirm-ai",
        );
    ...
    async updateConsentStatus(
        ...
        const response = await apiClient.patch<ApiResponse<ConfirmAIResponse>>(
            "/users/me/confirm-ai",
            request,
        );
        if (!response.data?.success || !response.data.data) {
    ...
    async summarizeConversation(
        ...
        const response = await apiClient.post<ApiResponse<AISummarizeResponse>>(
            "/ai/summarize",
            request,
        );
        if (!response.data?.success || !response.data.data) {
